...
const NotifDropdown = ({ onClose }) => {
  ...
  const handleItemClick = async (n) => {
    if (!n.read) {
      try { await markAsRead(n.id) } catch { /* silent */ }
    }
    onClose()
  ...
  return (
    ...
    >
      ...
      <div className="px-4 py-3 border-t border-gray-100 bg-gray-50">
        <button
          onClick={() => { onClose(); navigate('/admin/my-notifications') }}
...
const Navbar = ({ onMenuClick, isCollapsed }) => {
  ...
  // ✅ unreadCount context se — jab bhi notification read hoti hai, ye auto-update hota hai
  ...
  // Bell pulse jab naya unread notification aaye
  useEffect(() => {
    if (unreadCount > 0) {
      setBellPulse(true)
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <div ref={notifRef} className="relative">
            <button
              onClick={() => { setIsNotifOpen(p => !p); setIsUserMenuOpen(false) }}
              title="Notifications"
            ...
            >
              ...
              />
              {/* ✅ Badge — unreadCount = 0 hone par auto hide */}
              {unreadCount > 0 && (
                <span
            ...
            {/* ✅ Mini dropdown with read functionality */}
            {isNotifOpen && (
              <NotifDropdown onClose={() => setIsNotifOpen(false)} />
            )}
          </div>
            ...
            {isUserMenuOpen && (
              ...
              >
                ...
                <div className="p-2 space-y-0.5">
                  ...
                  {/* ✅ My Notifications — badge from context */}
                  <button
                  ...
                  {/* Settings */}
                  <button
                    onClick={() => { setIsUserMenuOpen(false); navigate('/admin/settings/role-permissions') }}
                    className="w-full flex items-center gap-3 px-3 py-2.5 text-sm text-gray-700 hover:bg-gray-50 rounded-xl transition-colors group"
                  >
