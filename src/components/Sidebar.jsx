...
const Sidebar = ({ isOpen, onClose, onToggleCollapse, isCollapsed }) => {
  ...
  const { hasPermission }     = useAuth()      // ✅ pull from context
  ...
  // ── Permission filters ──────────────────────────────────────────────────
  const canSee      = (item)       => hasPermission(item.permission ?? null)
  const visibleSubs = (subs = [])  => subs.filter(s => hasPermission(s.permission ?? null))
  const canSeeGroup = (group)      => group.items.some(item => canSee(item))
  ...
  useEffect(() => {
    ...
    sidebarMenuItems.forEach(entry => {
      ...
      entry.items.filter(canSee).forEach(item => {
        ...
        if (hit || item.path === location.pathname) {
          if (hit) drops[item.id] = true
          grps[entry.id] = true
        }
      })
    })

    setOpenDropdowns(drops)
    setOpenGroups(prev => ({ ...prev, ...grps }))
  ...
  const renderItem = item => {
    if (!canSee(item)) return null    // ✅ PERMISSION GATE
    ...
    const isActive       =
      ...
      filteredSubs.some(s => s.path === location.pathname || location.pathname.startsWith(s.path))
                  ...
                  return (
                    <NavLink
                      ...
                      className={({ isActive: na }) =>
                        ...
                        ${na || subActive ? COLORS.primary : `${COLORS.muted} hover:text-gray-600`}`
                      }
                    >
                    ...
                    return (
                      <NavLink
                        ...
                        className={({ isActive: na }) =>
                          ...
                          ${na || subActive ? COLORS.primary : 'text-gray-500 hover:text-gray-800'}`
                        }
                      >
  ...
  const renderGroup = group => {
    if (!canSeeGroup(group)) return null    // ✅ hide entire group if no items visible
