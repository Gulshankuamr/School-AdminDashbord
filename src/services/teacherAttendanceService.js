...
export const teacherAttendanceService = {
  ...
  getTeacherAttendanceByDate: async (date) => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/schooladmin/getTeacherAttendance?date=${date}`,
        {
          method: 'GET',
          headers: { 
            'Authorization': `Bearer ${token}`,
      ...
      if (!response.ok || !data.success) {
        throw new Error(data.message || 'Failed to fetch teacher attendance')
      }
  ...
  // ✅ FIXED: Update teacher attendance (केवल ये fields भेजें)
  updateTeacherAttendance: async (attendanceData) => {
    try {
      ...
      // ✅ FIXED: केवल ये 3 fields भेजें जैसे API को चाहिए
      const payload = {
