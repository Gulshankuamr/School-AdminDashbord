...
const EditTeacher = () => {
  ...
  useEffect(() => {
    const fetchTeacher = async () => {
      ...
      try {
        ...
        console.log('📋 Edit - teacher data received:', data)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.teacher_photo instanceof File) {
        payload.teacher_photo = formData.teacher_photo
      }
      if (formData.aadhar_card instanceof File) {
        payload.aadhar_card = formData.aadhar_card
      }

      console.log('📤 Submitting update for teacher:', formData.teacher_id)
          ...
          <div className="bg-white border border-emerald-200 shadow-2xl rounded-xl overflow-hidden">
            <div className="bg-emerald-500 px-4 py-2 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-white font-semibold text-sm">Teacher Updated Successfully!</span>
              </div>
              <button onClick={() => setSaveSuccess(false)} className="text-white hover:text-emerald-200">
                <X className="w-4 h-4" />
              </button>
            </div>
            <div className="p-4">
              <p className="text-gray-700 text-sm">
                <span className="font-bold">{formData.name}</span>'s information has been updated. Redirecting...
              </p>
            </div>
          </div>
        </div>
                      ...
                      {filePreviews[file.name] ? (
                        filePreviews[file.name].startsWith('data:') || filePreviews[file.name].startsWith('http') ? (
                          <>
                            ...
                            />
                            <span className="text-xs text-emerald-600 font-medium">Click to replace</span>
                          </>
                        ) : (
                          <>
