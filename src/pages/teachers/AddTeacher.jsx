        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            ...
            <div className="p-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1.5">Qualification <span className="text-red-400">*</span></label>
                <div className="relative">
                  <div className={iconClass}><GraduationCap className="h-4 w-4 text-gray-400" /></div>
                  <input type="text" name="qualification" value={formData.qualification} onChange={handleChange} className={inputClass} required placeholder="e.g., M.Ed, B.Sc" />
                </div>
              </div>
                    ...
                    <label htmlFor={file.name} className="cursor-pointer flex flex-col items-center justify-center h-full p-4">
                      {filePreviews[file.name] ? (
                        typeof filePreviews[file.name] === 'string' && filePreviews[file.name].startsWith('data:') ? (
                          <>
                            <img src={filePreviews[file.name]} alt="Preview" className="w-20 h-20 object-cover rounded-lg mb-2 shadow border border-emerald-200" />
                            <span className="text-xs text-emerald-600 font-semibold bg-emerald-50 px-2 py-0.5 rounded-full">✓ Uploaded</span>
                          </>
                        ) : (
                          <>
                            <div className="bg-emerald-100 rounded-full p-2 mb-2"><CheckCircle className="w-5 h-5 text-emerald-600" /></div>
                        ...
                        <>
                          ...
                          <span className="text-xs text-gray-400 text-center mt-0.5">JPG, PNG{file.accept.includes('pdf') ? ', PDF' : ''} (Max 5MB)</span>
                        </>
                      )}
                    </label>
                  </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-end pb-6">
            <button type="button" onClick={resetForm} disabled={loading}
              className="px-6 py-2.5 bg-white border border-gray-200 text-gray-600 rounded-lg hover:bg-gray-50 transition font-medium text-sm order-3 sm:order-1">
              Reset
            </button>
            <button type="button" onClick={() => navigate('/admin/teachers')} disabled={loading}
              className="px-6 py-2.5 bg-white border border-gray-200 text-gray-600 rounded-lg hover:bg-gray-50 transition font-medium text-sm order-2">
              Cancel
            </button>
            <button type="submit" disabled={loading}
              className={`px-6 py-2.5 bg-emerald-600 text-white rounded-lg font-medium text-sm flex items-center justify-center gap-2 order-1 sm:order-3 shadow-sm hover:bg-emerald-700 transition ${loading ? 'opacity-70 cursor-not-allowed' : ''}`}>
              {loading ? (
                <>
